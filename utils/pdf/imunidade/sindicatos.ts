...
export interface ImunidadeSindicatosFormData extends BasePdfFormData {
...
function renderDocs(data: ImunidadeSindicatosFormData) {
  const docs = [
    { label: "Estatuto", value: data.docEstatuto },
    ...
    { label: "Croqui", value: data.docCroqui },
  ...
  const anexados = data.documentosAnexados?.map((doc) => ({ label: doc, value: true })) || [];
  const all = [...docs, ...anexados];

  if (!all.length) {
    return `<div class="info-item"><div class="info-label">Documentos</div><div class="info-value">Vide anexos</div></div>`;
  }
...
function generatePdfHtml(data: ImunidadeSindicatosFormData): string {
...
<head>
  <meta charset="UTF-8">
  <title>Requerimento Digital de Imunidade – Sindicatos</title>
...
<body>
  <div class="header">
    ...
    <div class="header-text">
      ...
      <h3>Requerimento Digital de Imunidade – Sindicatos</h3>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="info-grid">
      ...
      <div class="info-item"><div class="info-label">Comprovante</div><div class="info-value">${data.possuiComprovanteTaxa ? "Anexado" : "Não anexado"}</div></div>
    </div>
  </div>
...
export async function generatePdfImunidadeSindicatos(
  data: ImunidadeSindicatosFormData
): Promise<Buffer> {
...
export async function getImunidadeSindicatosFileName(
  data: ImunidadeSindicatosFormData
): Promise<string> {
  const nomeLimpo = sanitizeFileName(data.nome || "Requerimento_Sindicatos");
  return `Requerimento_Imunidade_Sindicatos_${nomeLimpo}.pdf`;
}
