...
function generatePdfHtml(data: ImunidadeInstituicoesFormData): string {
...
<head>
  <meta charset="UTF-8">
  <title>Requerimento Digital de Imunidade – Instituições sem Fins Lucrativos</title>
  <style>
    ${baseStyles}
  </style>
</head>
<body>
  <div class="header">
    ...
    <div class="header-text">
      ...
      <h3>Requerimento Digital de Imunidade – Instituições sem Fins Lucrativos</h3>
    </div>
  </div>
...
export async function generatePdfImunidadeInstituicoes(
  data: ImunidadeInstituicoesFormData
): Promise<Buffer> {
...
export async function getImunidadeInstituicoesFileName(
  data: ImunidadeInstituicoesFormData
): Promise<string> {
  const nomeLimpo = sanitizeFileName(data.nome || "Requerimento_Instituicoes");
  return `Requerimento_Imunidade_Instituicoes_${nomeLimpo}.pdf`;
}
