...
export interface ImovelCedidoFormData extends BasePdfFormData {
...
<body>
  ...
  <div class="section">
    ...
    <div class="info-grid">
      ...
      ${
        data.inscricaoMercantil
          ? `
      <div class="info-item">
        <div class="info-label">Inscrição Mercantil</div>
        <div class="info-value">${formatValue(data.inscricaoMercantil)}</div>
      </div>
      `
          : ""
      }
  ...
  <div class="section">
    ...
    <div class="info-grid">
      <div class="info-item">
        ...
        <div class="info-value">${formatValue(data.nomeInquilino)}</div>
      </div>
      <div class="info-item">
        <div class="info-label">CPF</div>
        <div class="info-value">${formatValueOptional(data.cpfInquilino)}</div>
      </div>
      <div class="info-item">
        <div class="info-label">RG</div>
        <div class="info-value">${formatValueOptional(data.rgInquilino)}</div>
      </div>
      <div class="info-item">
        <div class="info-label">Órgão Emissor</div>
        <div class="info-value">${formatValueOptional(data.orgaoEmissorInquilino)}</div>
      </div>
      <div class="info-item">
        <div class="info-label">Telefone</div>
        <div class="info-value">${formatValueOptional(data.telefoneInquilino)}</div>
      </div>
      <div class="info-item">
        <div class="info-label">E-mail</div>
        <div class="info-value">${formatValueOptional(data.emailInquilino)}</div>
      </div>
    </div>
  </div>
  `
  ...
  <div class="declaration-section">
    ...
    <div class="declaration-info">
      ...
      <div class="declaration-item">
        <span class="declaration-label">REQUERENTE:</span>
        <span class="declaration-value">${formatValue(data.nome)}</span>
      </div>
    </div>
    <div class="declaration-footer">
      O requerente concordou eletronicamente com os termos acima no momento do envio do requerimento.<br>
...
export async function generatePdfImovelCedido(data: ImovelCedidoFormData): Promise<Buffer> {
  const htmlContent = generatePdfHtml(data, "Requerimento Digital de Isenção - Imóvel Cedido");
...
export async function getImovelCedidoFileName(data: ImovelCedidoFormData): Promise<string> {
  const nomeLimpo = sanitizeFileName(data.nome || "Requerimento");
  return `Requerimento_ImovelCedido_${nomeLimpo}.pdf`;
}

// Exportação genérica para templo religioso e taxas mercantis (que usam mesmo layout)
export async function generatePdfGenerico(
  data: ImovelCedidoFormData,
  titulo: string
): Promise<Buffer> {
  const htmlContent = generatePdfHtml(data, titulo);
...
export async function getGenericoFileName(
  data: ImovelCedidoFormData,
  prefixo: string
): Promise<string> {
  const nomeLimpo = sanitizeFileName(data.nome || "Requerimento");
  return `${prefixo}_${nomeLimpo}.pdf`;
}
